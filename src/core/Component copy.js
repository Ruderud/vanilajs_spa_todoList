...
export default class Component {
  ...
  render() {
    ...
    // const newEl = genNode(this.HTMLtemplate());
    // const oldEl = genNode(this.target.innerHTML);

    const newDom = document
      .createRange()
      .createContextualFragment(this.HTMLtemplate().trim());

    console.log("frag", newDom.firstChild);

    // const test = document.createElement("div");
    // test.innerText = "fuck";
    // $root.appendChild(test);
    //
    // setTimeout(() => updateElement($root, newNode, oldNode), 2000); // 1초 뒤에 DOM 변경

    updateElement(this.target, newDom.firstChild, this.target.firstChild);
    // this.target.appendChild(oldNode);

    // console.log(this.target);
    // const oldDom = document
    //   .createRange()
    //   .createContextualFragment(this.target?.innerHTML);
    // const newDom = document
    //   .createRange()
    //   .createContextualFragment(this.HTMLtemplate());
    // // console.log("this.target.children", this.target.children);

    // // update(this.target, newDom);

    // const oldNode = copyRender(this.HTMLtemplate());
    // const newNode = copyRender(this.HTMLtemplate());

    // copyUpdateElement(this.target, oldNode, newNode);
    // // updateElement(this.target, oldDom, newDom);
...
function updateElement(parent, newNode, oldNode) {
  ...
  // console.log("자식태그이름이 같음");
  updateAttributes(oldNode, newNode);

  const newChildren = [...newNode.childNodes];
  const oldChildren = [...oldNode.childNodes];
  const maxLength = Math.max(newChildren.length, oldChildren.length);
  // console.log("손자태그 비교들어감");
